...
function App() {
  ...
  const shuffleCards = () => {
    const animals = ['cat', 'dog', 'lion', 'tiger', 'elephant', 'giraffe', 'panda', 'zebra'];
  ...
  const isCardFlipped = (card) => flippedCards.includes(card) || matchedCards.includes(card);
    ...
    <div className="App">
      ...
      <div className="game-board">
        {cards.map((card) => (
          ...
          >
            {isCardFlipped(card) && <img src={`images/${card.animal}.jpeg`} alt={card.animal} />}
          </div>
        ))}
      </div>
